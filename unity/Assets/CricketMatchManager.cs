...
{ 
	...
	// --- HIDE THE COACH ---
        if (coachModel != null) coachModel.SetActive(false);
	...
	if (targetScore > 0 && runsScored >= targetScore)
{
    ...
    public void PlayCoachDemo()
{
    if (coachModel != null) coachModel.SetActive(true); // Make sure he is visible
    if (coachAnimator == null) return;

    // Reset all triggers so animations don't stack up
    coachAnimator.ResetTrigger("ShowCoverDrive");
    coachAnimator.ResetTrigger("ShowSweep");
    coachAnimator.ResetTrigger("ShowUpperCut");

    // Trigger the correct animation based on the UI selection
    switch (currentPracticeShot)
    {
        case PracticeShot.CoverDrive:
            ...
            break;
        case PracticeShot.Sweep:
            coachAnimator.SetTrigger("ShowSweep");
            break;
        case PracticeShot.UpperCut:
            coachAnimator.SetTrigger("ShowUpperCut");
            break;
    }
    
    Debug.Log($"🧑‍🏫 Coach is demonstrating the {currentPracticeShot}!");
}
...
void ShowCoachFeedback(string feedback)
{
    ...
    try
    {
        ...
        // --- NEW: SHOW COACH AND DEMONSTRATE FIX ---
        if (parsed.feedback != null && !parsed.feedback.Contains("Perfect"))
        {
            // Bring the coach back to the screen
            if (coachModel != null) coachModel.SetActive(true);

            if (coachAnimator != null)
            {
                // Play the animation of the shot you were *supposed* to play
                PlayCoachDemo(); 
            }
        }
    }
